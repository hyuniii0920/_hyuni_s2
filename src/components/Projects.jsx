...
export const featured = [
  {
    ...
    detail: {
      ...
      role: {
        title: 'AI 파이프라인 기획 · PM',
        items: [
          'OCR · LLM · RAG 분석 파이프라인 요구사항 정의 및 아키텍처 설계',
          'Safety Score 채점 정책 설계 (일관성 · 재현성 확보)',
          '외부 AI 장애 대응 리스크 관리 및 폴백 정책 수립',
          '스캔 문서 처리 성능 개선 목표 설정 및 검증 (8배 단축)',
        ],
      },
      github: [
      ...
      problemSolving: [
        {
          ...
          process: '점수 산출 책임을 AI가 아닌 백엔드로 이관하기로 결정 — LLM은 위험도 · 신뢰도 신호만 제한된 값으로 반환하게 하고, 실제 감점 계산은 가중치 기반 정책으로 백엔드가 수행하도록 역할을 재설계. 고위험 조항이 일정 개수 이상이면 추가 패널티를 부여하는 100점 기준 채점 정책을 수립.',
        ...
        },
        {
          ...
          process: '계약서 조항을 단위별로 관리하도록 데이터 구조를 재설계하고, AI 응답이 근거로 삼은 조항 원문을 항상 함께 반환하도록 응답 정책을 재구성.',
        ...
        },
        {
          ...
          process: '요청 처리와 백그라운드 분석 작업의 생명주기가 서로 다르다는 점을 원인으로 파악하고, 분석 작업이 요청과 독립적으로 안전하게 실행 · 종료되도록 처리 구조를 재설계.',
        ...
        },
        {
          ...
          process: '장애 발생 시나리오를 정의하고, 외부 AI 호출이 실패할 경우 주요 위험 유형을 키워드 기반으로 재탐지하는 대체 로직으로 자동 전환되도록 정책을 설계.',
  ...
  },
  {
    ...
    detail: {
      ...
      role: {
        title: '프론트엔드 · 프로덕트 기획',
        items: [
          '멀티테넌트 SaaS 아키텍처 요구사항 정의 (B2B/B2C 동시 지원)',
          '6개 직급 이해관계자 요구사항 통합 및 화면 우선순위 조율',
          '테넌트 온보딩 비용 최소화 정책 설계 (코드 수정 없는 확장 구조)',
          '동시 요청 상황의 인증 안정성 정책 수립',
          'Figma 기반 기획 문서화 및 AI 활용 프로토타이핑',
        ],
      },
      github: [
      ...
      problemSolving: [
        {
          ...
          process: '테넌트를 URL 기반으로 자동 식별하도록 설계하고, 브랜딩(로고 · 컬러)과 활성화 기능(장바구니, 커뮤니티, 유료 모드 등)을 설정값 기반으로 런타임에 반영하도록 정책을 재설계.',
        ...
        },
        {
          ...
          process: '토큰 갱신이 이미 진행 중이면 이후 요청은 대기열에 쌓아두고, 갱신이 끝난 뒤 한 번에 재실행하는 방식으로 인증 처리 정책을 재설계.',
        ...
        },
        {
          ...
          process: 'B2B/B2C 흐름을 화면 단위로 완전히 분리하되, 6개 직급이 하나의 프론트엔드 안에서 각자의 화면 · 권한으로 독립 동작하도록 구조를 통합 설계. 직급별로 모듈을 나누고 진입 시점에만 흐름이 갈리도록 구성해 조건 분기를 최소화.',
        ...
        },
        {
          ...
          process: '도메인 단위로 데이터 캐시 규칙을 통일하고, 갱신 범위를 계층적으로 통제할 수 있도록 데이터 패칭 정책을 재설계.',
  ...
  },
  {
    ...
    detail: {
      ...
      role: {
        title: '기획 · AI',
        items: [
          '반자동 콘텐츠 제작 파이프라인 기획 및 검증',
          '멤버 단위 콘텐츠 접근 권한 정책 설계',
          '저장 · 스트리밍 인프라 구조 트레이드오프 판단',
        ],
      },
      ...
      problemSolving: [
        {
          ...
          process: '완전 자동 감지 대신, 작업자가 첫 프레임에서 멤버를 한 번만 지정하면 이후 프레임은 자동으로 추적되는 반자동 방식을 채택하기로 결정.',
        ...
        },
        {
          ...
          process: '곡 · 멤버 단위로 콘텐츠와 접근 권한을 함께 관리하는 데이터 구조를 설계하고, 결제 시스템과 연동해 멤버 단위로 잠금 해제 여부를 판단하도록 정책을 구현.',
        ...
        },
        {
          ...
          process: '영상 저장과 스트리밍 역할을 서로 다른 인프라로 분리하기로 결정하고, 백엔드가 항상 최신 스트리밍 주소를 안내하도록 역할을 재설계.',
        ...
        },
        {
          ...
          process: '멤버 지정 → 자동 추적 → 마스크 생성까지 이어지는 반자동 제작 워크플로우를 설계해, 작업자 개입을 첫 프레임 지정 한 번으로 최소화.',
  ...
  },
  {
    ...
    detail: {
      ...
      role: {
        title: '기획 · Android',
        items: [
          '전시 안내 서비스 사용자 흐름 기획 및 정보 구조 설계',
          'Offline-First 정책 설계 (네트워크 불안정 대응)',
          '4개국어 지원 요구사항 정의 및 전환 흐름 설계',
          '현장 운영 변경에 대응하는 QR 인식 정책 수립',
        ],
      },
      github: [
      ...
      problemSolving: [
        {
          ...
          process: '스캔 요청과 실제 인식 처리를 이중으로 통제하도록 제어 로직을 설계하고, 인식 작업을 화면과 분리된 백그라운드에서 처리해 사용자 경험이 끊기지 않도록 구현.',
        ...
        },
        {
          ...
          process: '앱 최초 실행 시 기본 데이터를 로컬에 미리 저장해두고, 네트워크 호출이 실패하면 자동으로 로컬 데이터를 사용하도록 폴백 정책을 설계.',
        ...
        },
        {
          ...
          process: '선택한 언어를 즉시 저장하고, 언어 전환 시 해당 언어 데이터로 전체 화면이 일관되게 갱신되도록 전환 흐름을 설계.',
        ...
        },
        {
          ...
          process: '여러 QR 코드 형식을 순서대로 시도해 해석하는 4단계 폴백 정책을 설계해, 운영팀이 형식을 바꿔도 앱이 유연하게 대응하도록 구성.',
      ...
      problemSolving: [
        {
          ...
          process: '세 서비스를 하나의 저장소에서 통합 관리하기로 결정하고, 프론트엔드 · 백엔드 배포를 각각 자동화해 버전 불일치 위험을 줄임.',
        ...
        },
        {
          ...
          process: '인증과 데이터 관리를 플랫폼 공통 구조로 통합 설계해, Android 앱과 웹이 동일한 분석 결과를 공유하도록 데이터 흐름을 구성.',
        ...
        {featured.map((p, i) => (
          ...
          >
            ...
            <div className={styles.featuredInfo}>
              ...
              <span className={styles.featuredDetailLink}>Details <b>→</b></span>
            </div>
          </button>
        ))}
      </div>
